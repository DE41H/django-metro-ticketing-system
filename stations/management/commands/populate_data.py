...
# Import the original populate logic and data
import csv
from io import StringIO
# ... [Other necessary imports like re, etc., if any]

from stations.models import Station, Line
# ----------------------------------------------------------------------
# (1) PASTE YOUR EMBEDDED DATA HERE (LINES_CSV, STATIONS_CSV, COLOR_MAP)
# ----------------------------------------------------------------------

# --- Embedded Data from lines.csv ---
LINES_CSV = """
name,stations
...
# --- Embedded Data from stations.csv ---
STATIONS_CSV = """
uid,name,neighbours
1,AIIMS,29|52
2,Adarsh Nagar,67|8
...
# Dictionaries for mapping and data storage
uid_to_name_map = {}
...
def run_populate_logic():
    # This is the original logic that was in your populate_metro_data.py
    # NOTE: Since this function is only called from handle() within the Command,
    ...
    # ====================================================================
    print("\n[CLEANUP] Deleting existing database records...")
    Station.objects.all().delete()
    Line.objects.all().delete()
    print("Clean up complete.")

    # ====================================================================
    # PASS 0: ROBUST PRE-PROCESS (STATIONS_CSV)
    # ====================================================================
    print("\n[PASS 0] Pre-processing UIDs to Names (STATIONS_CSV)...")

    lines = [line.strip() for line in STATIONS_CSV.strip().split('\n') if line.strip()]
    data_lines = lines[1:] # Skip header

    for line in data_lines:
        # Robust handling for CSV lines with embedded commas (e.g., station names)
        if '"' in line:
    ...
    print(f"  - Created UID-to-Name map for {len(uid_to_name_map)} Stations.")


    # ====================================================================
    # PASS 1A: CREATE ALL STATION OBJECTS
    # ====================================================================
    print("\n[PASS 1A] Creating Station objects...")
    stations_to_create = []

    for name in uid_to_name_map.values():
        stations_to_create.append(Station(pk=name, name=name)) 
    ...
    print(f"Successfully created {len(name_to_station_object)} Station records.")


    # ====================================================================
    # PASS 1B: CREATE ALL LINE OBJECTS
    # ====================================================================
    print("\n[PASS 1B] Creating Line objects (LINES_CSV)...")
    lines_to_create = []

    # Robust parsing logic for LINES_CSV
    lines = [line.strip() for line in LINES_CSV.strip().split('\n') if line.strip()]
    data_lines = lines[1:] # Skip header
    ...
    for line in data_lines:
        ...
        raw_line_data.append({
            ...
            'station_uids': raw_stations
        })

    Line.objects.bulk_create(lines_to_create)
    name_to_line_object = {line.name: line for line in lines_to_create}

    print(f"Successfully created {len(name_to_line_object)} Line records.")


    # ====================================================================
    # PASS 2: SET M2M RELATIONSHIPS
    # ====================================================================
    print("\n[PASS 2] Setting Many-to-Many relationships...")

    # --- 2A: Line to Station M2M ---
    print("  - Line-to-Station relationships...")
    for data in raw_line_data:
        line_obj = name_to_line_object[data['line_object'].pk]
        station_uids = data['station_uids']
        
        station_names = [uid_to_name_map[uid] for uid in station_uids if uid in uid_to_name_map]
        station_objects = [name_to_station_object[name] for name in station_names if name in name_to_station_object]
        
        line_obj.stations.set(station_objects)

    print(f"  - Set station relationships for {len(raw_line_data)} Lines.")


    # --- 2B: Station to Station (Neighbours) M2M ---
    print("  - Station-to-Station (Neighbours) relationships...")
    for station_name, neighbor_uids in raw_station_m2m_data.items():
        if station_name in name_to_station_object:
            station_obj = name_to_station_object[station_name]
            
            neighbor_names = [uid_to_name_map[n_uid] for n_uid in neighbor_uids if n_uid in uid_to_name_map]
            neighbor_objects = [name_to_station_object[name] for name in neighbor_names if name in name_to_station_object]
            
            station_obj.neighbours.set(neighbor_objects)

    print(f"  - Set neighbour relationships for {len(name_to_station_object)} Stations.")
...
# ----------------------------------------------------------------------------------
# (2) The Custom Management Command Class
# ----------------------------------------------------------------------------------

class Command(BaseCommand):
    help = 'Populates the database with initial metro station and line data.'

    @transaction.atomic
    def handle(self, *args, **options):
        # Call the logic function to run the data population
        run_populate_logic()
